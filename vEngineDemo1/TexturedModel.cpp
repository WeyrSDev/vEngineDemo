...
namespace Rendering
{
	RTTI_DEFINITIONS(TexturedModel)

		TexturedModel::TexturedModel(Engine& game, Camera& camera)
	...
	void TexturedModel::Initialize()
	{
		...
		if (mPass == nullptr)
		{
			throw Exception("ID3DX11EffectTechnique::GetPassByName() could not find the specified pass.", hr);
		}
	...
	void TexturedModel::Draw(const Time& gameTime)
	{
	...
	void TexturedModel::CreateVertexBuffer(ID3D11Device* device, const Mesh& mesh, ID3D11Buffer** vertexBuffer) const
	{
